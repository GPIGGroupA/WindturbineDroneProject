...
{
    public ChargingPad(pad_state p_state= pad_state.Empty, float p_battery_percentage= 0F)
    {
...
public class HubTurbine : WindTurbine
{
    ...
    void Update()
    {
        ...
        for (int i=0; i<charging_pads.Length; i++){
            if (charging_pads[i].state==pad_state.DelivaryDrone || charging_pads[i].state==pad_state.MaintainenceDrone){

                if (charging_pads[i].battery_percentage < 100F) {
                    charging_pads[i].battery_percentage+= chargeRate;
                    if (charging_pads[i].battery_percentage > 100F){charging_pads[i].battery_percentage= 100F;}
                }
    ...
    bool ReleaseDrone(int ind)
    {
        switch (charging_pads[ind].state) 
        {
            case pad_state.DelivaryDrone:
                Instantiate(delivary_drone_prefab, this.transform.position + new Vector3(0, 105, 0), Quaternion.identity);
                break;

            case pad_state.MaintainenceDrone:
                Instantiate(maintenance_drone_prefab, this.transform.position + new Vector3(0, 105, 0), Quaternion.identity);
                break;
